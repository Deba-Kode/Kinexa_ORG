...
//                                         <img src={`http://192.168.0.116:3001/uploads/posts/${post._id}/${post.imageUpload}`} className="card-img-top mt-2 img-hover-zoom" alt="..." style={{ cursor: "pointer", transition: "transform 0.5s", borderRadius: "0.5rem" }} onMouseOver={(e) => e.target.style.transform = "scale(1.1)"} onMouseOut={(e) => e.target.style.transform = "scale(1.0)"} />
...
const ParticularUserPostsData = () => {
    const { loading, error, data } = useQuery(GET_PARTICULAR_USER);
    const { loading: postLoading, error: postError, data: UserPosts, refetch } = useQuery(GET_ALL_POSTS);
    const [userData, setUserData] = useState({
        profilePic: '',
        coverPic: ''
    });
    ...
    const deletePost = async (postId) => {
        try {
            await axios.delete(`http://192.168.0.116:3001/delete-post/${postId}`);
                                ...
                                .map(post => (
                                    <div key={post._id} className="card m-3 mt-3 mx-auto" style={{ maxWidth: "14rem" }}>
                                        ...
                                        <div className="card-body">
                                            <p className='m-0'><span className='fw-bold'>Caption: </span>{post.caption}</p>
                                            <p className='m-1'><span className='fw-bold'>Likes: </span>{post.likeCount}</p>
                                            <i className="bi bi-trash-fill" style={{ color: "red", cursor: "pointer" }} onClick={() => deletePost(post._id)}></i>
                                        </div>
                                    </div>
                                ))}
                        </div>
...
export default ParticularUserPostsData;
